                        ...
                        <div class="pl-4">
                            ...
                            @php
                                if(Auth::check()){
                                    ...
                                    $followed = App\Models\Follower::where('shop_id', $shop->id)
                                                        ->where('user_id', $user_id)->first();
                                }
                            @endphp
                            <p action="{{@$followed? "unfollow":"follow"}}" sid="{{$shop->id}}" class="btn btn-styled btn-base-1 btn_follow">{{@$followed? "Unfollow":"Follow"}}</p>
                            <span class="count_followers">{{count($shop->followers)}} Followers</span>
                        </div>
                    </div>
                </div>
                <div class="col-md-6">
                ...
                <div class="col">
                    <div class="seller-shop-menu">
                        <ul class="inline-links">
                            <li @if(!isset($type)) class="active" @endif><a style="padding: 14px 10px;" href="{{ route('shop.visit', $shop->slug) }}">{{__('Timeline')}}</a></li>

                            <li @if(isset($type) && $type == 'home_store') class="active" @endif><a style="padding: 14px 10px;" href="{{ route('shop.visit.type', ['slug'=>$shop->slug, 'type'=>'home_store']) }}">{{__('Home Store')}}</a></li>

                            <li @if(isset($type) && $type == 'top_selling') class="active" @endif><a style="padding: 14px 10px;" href="{{ route('shop.visit.type', ['slug'=>$shop->slug, 'type'=>'top_selling']) }}">{{__('Top Selling')}}</a></li>

                            <li @if(isset($type) && $type == 'all_products') class="active" @endif><a style="padding: 14px 10px;" href="{{ route('shop.visit.type', ['slug'=>$shop->slug, 'type'=>'all_products']) }}">{{__('All Products')}}</a></li>
                        </ul>
                    </div>
                </div>
            </div>
    ...
    <section class="@if (@$type == 'home_store') gry-bg @endif pt-5">
        <div class="container">
            <div class="row">
                        		...
                        		<ul class="seller-brand-list">
                                    @foreach ($brands as $brand_id)
                                        @if (App\Models\Brand::find($brand_id) != null)
                                            <li class="brand-item">
                                                <a href="{{ route('products.brand', App\Models\Brand::find($brand_id)->slug) }}">
                                                    <img src="{{ asset('frontend/images/placeholder.jpg') }}" data-src="{{ asset(App\Models\Brand::find($brand_id)->logo) }}" class="img-fluid lazyload" alt="{{ App\Models\Brand::find($brand_id)->name }}">
                                                </a>
                                            </li>
                                        @endif
                                    @endforeach
                ...
                {{-- Timeline --}}
                @if (!isset($type))
                    @include('AccountPanel.seller.timeline')
                @endif
                @if (isset($type))
                <div class="col-xl-9">
                    ...
                    <div class="product-list row gutters-5 sm-no-gutters">
                        @php
                            if ($type == 'home_store'){
                                $products = App\Models\Product::where('user_id', $shop->user->id)->where('published', 1)->orderBy('created_at', 'desc')->paginate(12);
                            }
                            elseif ($type == 'top_selling'){
                                $products = App\Models\Product::where('user_id', $shop->user->id)->where('published', 1)->orderBy('num_of_sale', 'desc')->paginate(12);
                            }
                            elseif ($type == 'all_products'){
                                $products = App\Models\Product::where('user_id', $shop->user->id)->where('published', 1)->paginate(12);
                            }
                        @endphp
