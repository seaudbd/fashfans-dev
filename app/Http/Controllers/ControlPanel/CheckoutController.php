...
//        }
    ...
    public function store_delivery_info(Request $request)
    {
        if(Session::has('cart') && count(Session::get('cart')) > 0){
            ...
            $cart = $cart->map(function ($object, $key) use ($request) {
                if(\App\Models\Product::find($object['id'])->added_by == 'admin'){
                    if($request['shipping_type_admin'] == 'home_delivery'){
                        $object['shipping_type'] = 'home_delivery';
                        $object['shipping'] = \App\Models\Product::find($object['id'])->shipping_cost;
                    }
                    else{
                    ...
                    if($request['shipping_type_'.\App\Models\Product::find($object['id'])->user_id] == 'home_delivery'){
                        $object['shipping_type'] = 'home_delivery';
                        $object['shipping'] = \App\Models\Product::find($object['id'])->shipping_cost;
                    }
                    else{
                        $object['shipping_type'] = 'pickup_point';
                        $object['pickup_point'] = $request['pickup_point_id_'.\App\Models\Product::find($object['id'])->user_id];
                        $object['shipping'] = 0;
                    }
                }
                return $object;
    ...
    public function get_payment_info(Request $request)
    {
        ...
        session()->put('order_code', rand(10000, 99999));
