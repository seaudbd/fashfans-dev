    ...
    <div class="col-md-6">
        <div class="panel">
            <div class="panel-body text-center dash-widget dash-widget-left">
                ...
                <br>
                <p class="text-lg text-main">{{__('Total published products')}}: <span class="text-bold">{{ \App\Models\Product::where('published', 1)->get()->count() }}</span></p>
                @if (\App\Models\BusinessSetting::where('type', 'vendor_system_activation')->first()->value == 1)
                    <p class="text-lg text-main">{{__('Total sellers products')}}: <span class="text-bold">{{ \App\Models\Product::where('published', 1)->where('added_by', 'seller')->get()->count() }}</span></p>
                @endif
                <p class="text-lg text-main">{{__('Total admin products')}}: <span class="text-bold">{{ \App\Models\Product::where('published', 1)->where('added_by', 'admin')->get()->count() }}</span></p>
                <br>
